...
Slimemaid Source Code (02.05.2022)
...
void vulkan_instance_handler::initWindow() {}

void vulkan_instance_handler::initVulkan() {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
...
void vulkan_instance_handler::mainLoop() {
  std::cout << "Start main loop function..." << std::endl;
  while (!window->glfwHandleWindowShouldClose()) {
    glfwPollEvents();
    drawFrame();
  }
...
void vulkan_instance_handler::cleanUpSwapChain() {
...
void vulkan_instance_handler::cleanUp() {
  cleanUpSwapChain();
...
void vulkan_instance_handler::recreateSwapChain() {
  int width = 0, height = 0;
  window->glfwHandleGetFramebufferSize(&width, &height);
  while (width == 0 || height == 0) {
    window->glfwHandleGetFramebufferSize(&width, &height);
    window->glfwHandleWaitEvents();
  }
...
void vulkan_instance_handler::createInstance() {
...
void vulkan_instance_handler::setupDebugMessenger() {
...
void vulkan_instance_handler::createSurface() {
  if (VK_SUCCESS != window->glfwHandleCreateWindowSurface(instance, nullptr, &surface)) {
...
void vulkan_instance_handler::pickPhysicalDevice() {
  ...
  std::cout << "Picking physical device process ends with success..." << std::endl
            << "Chosen physical device: " << chosenDeviceProperties.deviceName << std::endl;
}

VkSampleCountFlagBits vulkan_instance_handler::getMaxUsableSampleCount() {
...
void vulkan_instance_handler::createLogicalDevice() {
  ...
  vkGetDeviceQueue(device, transferIndices.graphicsFamily.value(), 0, &graphicsQueue);
  vkGetDeviceQueue(device, transferIndices.presentFamily.value(), 0, &presentQueue);

  std::cout << "Logical device creation process ends with success..." << std::endl;
}

void vulkan_instance_handler::createSwapChain() {
...
void vulkan_instance_handler::createImageViews() {
...
void vulkan_instance_handler::createRenderPass() {
...
void vulkan_instance_handler::createDescriptorSetLayout() {
...
void vulkan_instance_handler::createGraphicsPipeline() {
...
void vulkan_instance_handler::createFramebuffers() {
  ...
  for (size_t i = 0; i < swapChainImageViews.size(); i++) {
    ...
    if (VK_SUCCESS != vkCreateFramebuffer(device, &framebufferCreateInfo, nullptr, &swapChainFramebuffers[i])) {
...
void vulkan_instance_handler::createCommandPool() {
  queue_family_indices queue_family_indices = findQueueFamilies(physicalDevice, surface);

  VkCommandPoolCreateInfo commandPoolCreateInfo{};
  commandPoolCreateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
  commandPoolCreateInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
  commandPoolCreateInfo.queueFamilyIndex = queue_family_indices.graphicsFamily.value();
...
void vulkan_instance_handler::createDepthResources() {
...
VkFormat vulkan_instance_handler::findDepthFormat() {
...
VkFormat vulkan_instance_handler::findSupportedDepthFormat(const std::vector<VkFormat> &candidates,
                                                           VkImageTiling inputTiling, VkFormatFeatureFlags inputFlags) {
...
void vulkan_instance_handler::createColorResources() {
  ...
  createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
              ...
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory, physicalDevice, device);
  colorImageView = createImageView(colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
}

void vulkan_instance_handler::createTextureImage() {
  ...
  copyBufferToImage(stagingBuffer, textureImage, static_cast<size_t>(textureWidth), static_cast<size_t>(textureHeight));
...
void vulkan_instance_handler::createTextureImageView() {
  textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);

  std::cout << "Filling texture image view process ends with success..." << std::endl;
}

void vulkan_instance_handler::transitionImageLayout(VkImage inputImage, VkFormat inputFormat,
  ...
  if (VK_IMAGE_LAYOUT_UNDEFINED == inputOldImageLayout && VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL == inputNewImageLayout) {
...
void vulkan_instance_handler::generateMipmaps(VkImage inputImage, VkFormat inputImageFormat, uint32_t inputTextureWidth,
  ...
  for (uint32_t i = 1; i < inputMipLevels; i++) {
    ...
    vkCmdBlitImage(commandBuffer, inputImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, inputImage,
...
VkImageView vulkan_instance_handler::createImageView(VkImage inputImage, VkFormat inputFormat,
                                                     VkImageAspectFlags inputAspectMask, uint32_t inputMipLevels) {
...
void vulkan_instance_handler::createTextureSampler() {
...
void vulkan_instance_handler::loadModel() {
...
void vulkan_instance_handler::copyBufferToImage(VkBuffer inputBuffer, VkImage inputImage, uint32_t inputWidth,
  ...
  vkCmdCopyBufferToImage(commandBuffer, inputBuffer, inputImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageRegion);

  endSingleTimeCommands(commandBuffer);
}

void vulkan_instance_handler::createVertexBuffer() {
...
void vulkan_instance_handler::createIndexBuffer() {
...
void vulkan_instance_handler::createUniformBuffers() {
...
void vulkan_instance_handler::createBuffer(VkDeviceSize inputSize, VkBufferUsageFlags inputUsage,
                                           VkMemoryPropertyFlags inputMemoryProperties, VkBuffer &buffer,
  ...
  memAllocateInfo.memoryTypeIndex =
      findMemoryType(physicalDevice, bufferMemRequirements.memoryTypeBits, inputMemoryProperties);
...
void vulkan_instance_handler::copyBuffer(VkBuffer inputSrcBuffer, VkBuffer inputDstBuffer,
...
void vulkan_instance_handler::createDescriptorPool() {
...
void vulkan_instance_handler::createDescriptorSets() {
...
void vulkan_instance_handler::createCommandBuffers() {
...
void vulkan_instance_handler::createSyncObjects() {
...
std::vector<const char *> vulkan_instance_handler::getRequiredExtensions() {
...
bool vulkan_instance_handler::checkValidationLayerSupport() {
  uint32_t layerCount;
...
VkCommandBuffer vulkan_instance_handler::beginSingleTimeCommands() {
  VkCommandBufferAllocateInfo commandBufferAllocateInfo{};
  commandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
  commandBufferAllocateInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
  commandBufferAllocateInfo.commandPool = commandPool;
...
void vulkan_instance_handler::endSingleTimeCommands(VkCommandBuffer inputCommandBuffer) {
...
VkShaderModule vulkan_instance_handler::createShaderModule(const std::vector<char> &code) {
...
int vulkan_instance_handler::rateDeviceSuitability(VkPhysicalDevice device) {
  VkPhysicalDeviceProperties deviceProperties;
  VkPhysicalDeviceFeatures deviceFeatures;
...
VkSurfaceFormatKHR vulkan_instance_handler::chooseSwapSurfaceFormat(
...
VkPresentModeKHR vulkan_instance_handler::chooseSwapPresentMode(
...
VkExtent2D vulkan_instance_handler::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
  if (std::numeric_limits<uint32_t>::max() != capabilities.currentExtent.width) {
    return capabilities.currentExtent;
  } else {
    int height;
    int width;
    window->glfwHandleGetFramebufferSize(&height, &width);
...
bool vulkan_instance_handler::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
bool vulkan_instance_handler::isDeviceSuitable(VkPhysicalDevice device) {
...
void vulkan_instance_handler::drawFrame() {
  ...
  if (VK_ERROR_OUT_OF_DATE_KHR == queuePresentResult || VK_SUBOPTIMAL_KHR == queuePresentResult || framebufferResized) {
...
void vulkan_instance_handler::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
...
void vulkan_instance_handler::updateUniformBuffer(uint32_t currentImage) {
...
vulkan_instance_handler::vulkan_instance_handler() {}

vulkan_instance_handler::~vulkan_instance_handler() {}

void vulkan_instance_handler::run() {
  initWindow();
  initVulkan();
  mainLoop();
  cleanUp();
