...
------------------------------------
  ...
  Component keeps object for access to command pool for graphics logical_device. Command buffers count equal to frame
  that rendering on flight:
    ...
    - Command buffers are objects used to record commands which can be subsequently submitted to
      a logical_device queue for execution. There are two levels of command buffers - primary command buffers,
